...
pub fn respawn_system(world: &mut World, rigidbodies: &mut RigidBodySet, screen_width: usize, screen_height: usize) {
    world
        ...
        .for_each(|(_, (handle, state, player))| {
            if ActorState::Dead == *state {
                ...
                if let Some(rigidbody) = rigidbodies.get_mut(*handle) {
                    let x = gc::random_float_range(0.0, screen_width as f32 / PHYSICS_PIXEL_SCALING);
                    let y = screen_height as f32 / PHYSICS_PIXEL_SCALING;
                    let pos = Vector2::new(x, y);

                    gc::console_log(&format!("respawning player: {} to {}", player.0, pos));

                    rigidbody.set_position(Isometry2::new(pos, 0.0), true);
